...
public class StaffCommand implements CommandExecutor, TabCompleter {
	...
	public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
		...
		if (sender.hasPermission(Permissions.WRITE_STAFF)) {
			if (args.length == 0) {
				ChatManager.joinStaff(player.getUniqueId());
				sender.sendMessage(Strings.NOW_STAFF);
				return true;
			}
			...
			player.chat("###staff " + stringBuilder);
		} else {
